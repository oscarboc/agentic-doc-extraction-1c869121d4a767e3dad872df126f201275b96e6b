...
2. Usa el campo 'numero_autorizacion' como ancla principal para separar y distinguir cada autorización.
   Puede aparecer con variantes como: "AUTORIZACION No.:", "AUTORIZACION No .:", "AUTORIZACIÓN No:", "AUTORIZACION N°"
   o diferencias de espacios/acentos; trata todas esas variantes como equivalentes.
3. El numero_autorizacion debe tener exactamente 14 dígitos. Si no cumple, pasa a la siguiente, no la incluyas.
4. Devuelve una lista de autorizaciones en el campo 'authorizations'.
5. En items, incluye cada servicio autorizado con su codigo_cups, cantidad, dias_tratamiento y descripcion.
""".strip()
...
class OpenAIExtractorService:
    def __init__(self, settings: Settings) -> None:
        ...
        logger.info(
            ...
            provider,
            self.primary_model,
            "ollama" if provider == "openai" and self.fallback_client else
            "openai" if provider == "ollama" and self.fallback_client else "none",
    ...
    async def extract_authorization(
        ...
        # Try primary
        try:
            return await self._call_llm(self.primary_client, self.primary_model, markdown)
            ...
            logger.info(
                "Attempting fallback LLM | model=%s", self.fallback_model
            )
            try:
                return await self._call_llm(
                    self.fallback_client, self.fallback_model, markdown
                )
    ...
    async def _call_llm(
        ...
        response = await client.responses.parse(
            ...
            text_format=AuthorizationResponse,
        )
